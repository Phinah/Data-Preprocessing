        ...
        for expression in expressions:
            ...
            if not img_path.exists():
                for ext in ['.png', '.jpeg', '.JPG', '.PNG']:
                    test_path = self.base_dir / f"{member_name}_{expression}{ext}"
                    if test_path.exists():
                        img_path = test_path
                        break
            ...
            # Save augmented images to Images/augmented/{member_name}/
...
if __name__ == "__main__":
    ...
    # Check if images exist or are empty, create samples if needed
    print("\nChecking for image files...")
    members_needing_samples = set()
    for member in team_members:
        for expr in ['neutral', 'smile', 'surprised']:
            img_path = processor.base_dir / f"{member}_{expr}.jpg"
    ...
    if members_needing_samples:
        print(f"\n⚠ Creating sample placeholder images for: {', '.join(members_needing_samples)}")
        for member in members_needing_samples:
            processor.create_sample_images(member)
        print("✓ Sample images created. Replace with actual photos when available.\n")
